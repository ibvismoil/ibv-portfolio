      ...
      <div className="grid gap-6 md:grid-cols-3">
        {projectData.map((project, index) => (
          <div key={index} className="border p-4 rounded-lg shadow">
            ...
            <img className='border rounded-2xl' src={project.img} alt="" />
            <p className="test text-sm my-2">{project.description}</p>
            <p className="text-xs mb-2">Topics: {project.tech.join(', ')}</p>
            <div className="flex gap-4 text-sm justify-between">
              <a href={project.link} target="_blank" rel="noopener noreferrer" className="flex gap-1"><LinkIcon/> Demo</a>
              <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex gap-1"><Github width='18px' height='18px' />GitHub</a>
            </div>
            <h3 className='text-red-500'>{project.error}</h3>
          </div>
        ))}
      </div>
